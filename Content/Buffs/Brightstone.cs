	...
	public override void UpdateEquips() {
        ...
        if (Player.velocity.Length() >= 1f && (Player.controlLeft || Player.controlRight || Player.controlJump || Player.velocity.Y >= 1f) && !Player.rocketFrame) {
            if (Main.rand.NextBool(6))
				Projectile.NewProjectile(Player.GetSource_Misc("Brightstone"), Player.Center.X, Player.Center.Y, 0, 0, ModContent.ProjectileType<Projectiles.Friendly.Brightstone>(), 0, 0, Player.whoAmI);
		}
	}
}