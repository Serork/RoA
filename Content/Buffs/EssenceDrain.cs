...
internal class EssenceDrainNPC : GlobalNPC {
    ...
    public override void DrawEffects(NPC npc, ref Color drawColor) {
        if (essenceDrain) {
            ...
            int dust = Dust.NewDust(npc.Center, 0, 0, DustID.Shadowflame, 0f, 0f, 0, Color.DarkViolet, 1.4f);
            Main.dust[dust].noGravity = true;
            Main.dust[dust].position = npc.Center + vector;
            Main.dust[dust].velocity = vector;
            if (_essenceDrainTimer % 10 == 0) {
                float dustCountMax = 10;
                int dustCount = 0;
                while (dustCount < dustCountMax) {
                    if (Main.rand.NextBool(1)) {
                        ...
                        vector = vector.RotatedBy(npc.velocity.ToRotation(), default);
                        dust = Dust.NewDust(npc.Center, 0, 0, DustID.Shadowflame, 0f, 0f, 0, Color.DarkViolet, 1.4f);
                        Main.dust[dust].noGravity = true;
                        Main.dust[dust].position = npc.Center + vector;
                        Main.dust[dust].velocity = npc.velocity * 0f + vector.SafeNormalize(Vector2.UnitY) * 0.8f * Main.rand.NextFloat(-1f, 1f);
    ...
    public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor) {
        if (essenceDrain) {
            ...
            Color color = drawColor.MultiplyRGB(Color.DarkViolet);
            ...
            spriteBatch.Draw(texture, npc.Top + Vector2.UnitY * offset.Y * 0.5f - Vector2.UnitY * texture.Height * 0.6f - Main.screenPosition, new Rectangle?(), 
                color, 0f, origin, npc.scale * 0.75f, SpriteEffects.None, 0);
        }
        return true;
    }
}