...
sealed class FeathersInABottle : NatureItem {
    ...
    internal sealed class FeathersInABottleExtraJump : ExtraJump {
        ...
        public override bool CanStart(Player player) => !player.GetModPlayer<BaseFormHandler>().IsInDruidicForm && !player.GetModPlayer<WreathHandler>().StartSlowlyIncreasingUntilFull && player.GetModPlayer<WreathHandler>().HasEnough(0.25f);
