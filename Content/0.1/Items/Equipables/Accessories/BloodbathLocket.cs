            ...
            if (!(player.dead || player.invis || player.ShouldNotDraw)) {
                ...
                for (float i2 = -MathHelper.Pi; i2 <= MathHelper.Pi; i2 += MathHelper.PiOver2) {
                    Main.spriteBatch.Draw(
                        ...
                        drawInfo.Position + (player.gravDir != 1 ? Vector2.UnitY * 6f : Vector2.Zero) + Vector2.UnitY * (-posOffset.Y + seatYOffset) - Main.screenPosition + new Vector2(0f, drawInfo.drawPlayer.gfxOffY)
                            +
