        ...
        if (!player.HoldingLMB()) {
            if (plr._charge > 0f) {
                ...
                for (int i = 0; i < k; i++) {
                    ...
                    int dust = Dust.NewDust(vector3 + Main.rand.RandomPointInArea(6f) - player.velocity * 3f + vector2 * 2f * Main.rand.NextFloat() - new Vector2(1f, 2f), 0, 0, 6, vector2.X * 2f, vector2.Y * 2f, 0, default(Color), 2.75f + 0.5f * Main.rand.NextFloat());
