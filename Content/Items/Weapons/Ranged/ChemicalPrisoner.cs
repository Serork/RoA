...
sealed class ChemicalPrisoner : ModItem {
    ...
    public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
        ...
        if (player.GetModPlayer<ChemicalPrisonerPlayer>().count > 3) {
            ...
            Projectile.NewProjectile(source, position.X, position.Y - 2, velocity.X * 0.8f, velocity.Y * 0.8f, ModContent.ProjectileType<ChemicalFlask>(), damage, knockback, player.whoAmI);
        }
        else {
