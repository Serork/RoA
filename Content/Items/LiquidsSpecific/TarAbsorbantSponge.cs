...
sealed class TarAbsorbantSponge : ModItem {
    ...
    public override void SetStaticDefaults() {
        ...
        ItemID.Sets.ShimmerTransformToItem[Type] = ModContent.ItemType<TarBucket>();
    ...
    public override void HoldItem(Player player) {
        if (!player.JustDroppedAnItem)//make sure the player hasn't dropped an item
        {
            ...
            if (tile.LiquidType == LiquidLoader.LiquidType<Tar>()) //if the cursor's liquid selected is example liquid, then run the following logic
            {
