...
sealed class HallowedVisor : NatureItem, ItemGlowMaskHandler.IDrawArmorGlowMask, IDoubleTap {
    ...
    public static void SetDrawSettings_Inner(Player player, ref Texture2D texture, ref Color color, ref PlayerDrawSet drawInfo) {
    ...
    void IDoubleTap.OnDoubleTap(Player player, IDoubleTap.TapDirection direction) {
        if (player.CanTransfromIntoDruidForm<HallowedVisor>(direction)) {
    ...
    public override void EquipFrameEffects(Player player, EquipType type) {
        ...
        if (!player.HasSetBonusFrom<HallowedVisor>()) {
            return;
        }
