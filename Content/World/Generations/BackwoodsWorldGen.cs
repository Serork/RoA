...
sealed class BackwoodsWorldGen : ModSystem {
    ...
    private BackwoodsBiomePass _backwoodsWorldGen;

    public override void ModifyWorldGenTasks(List<GenPass> tasks, ref double totalWeight) {
        _backwoodsWorldGen = new BackwoodsBiomePass(GENLAYERNAME, LAYERWEIGHT);

        int genIndex = tasks.FindIndex(genpass => genpass.Name.Equals("Corruption"));
        genIndex += 7;
        tasks.Insert(genIndex, _backwoodsWorldGen);
        genIndex += 3;
        tasks.Insert(genIndex, new PassLegacy(GENLAYERNAME3, _backwoodsWorldGen.BackwoodsLootRooms));

        genIndex = tasks.FindIndex(genpass => genpass.Name.Equals("Smooth World"));
        genIndex -= 2;
        tasks.Insert(genIndex, new PassLegacy(GENLAYERNAME2, _backwoodsWorldGen.BackwoodsCleanup));

        genIndex = tasks.FindIndex(genpass => genpass.Name.Equals("Micro Biomes"));
        genIndex -= 3;
        tasks.Insert(genIndex, new PassLegacy(GENLAYERNAME3, _backwoodsWorldGen.BackwoodsOtherPlacements));

        //tasks.Add(new PassLegacy(GENLAYERNAME3, _backwoodsWorldGen.BackwoodsAltarPlacement));
    }
}
...
sealed class BackwoodsBiomePass(string name, double loadWeight) : GenPass(name, loadWeight) {
    ...
    private ushort _fallenTreeTileType, _plantsTileType, _bushTileType, _elderWoodChestTileType, _altarTileType, _mintTileType;
    ...
    public int CenterY {
        ...
        private set {
            _positionToPlaceBiome.Y = Math.Clamp(value, (int)GenVars.worldSurface - 100, (int)Main.rockLayer);
        }
    }

    public int Top => CenterY - _biomeHeight;
    public int Bottom => CenterY + _biomeHeight;
    public int Left => CenterX - _biomeWidth;
    public int Right => CenterX + _biomeWidth;
    public Point TopLeft => new(Left, Top);
    public Point TopRight => new(Right, Top);
    public int EdgeX => _biomeWidth / 4;
    public int EdgeY => _biomeHeight / 4;

    public ushort AltarPlaceholderTileType => TileID.ChlorophyteBrick;
    public ushort AltarPlaceholderTileType2 => TileID.Adamantite;
    ...
    private void Step16_PlaceAltar() {
        ...
        for (int i = tileX - 40; i < tileX + 40; i++) {
            for (int j = tileY - 40; j < tileY + 40; j++) {
                ...
                if (tile.ActiveTile(ModContent.TileType<TreeBranch>()) && !WorldGenHelper.GetTileSafely(i - 1, j).ActiveTile(treeBranch) && !WorldGenHelper.GetTileSafely(i + 1, j).ActiveTile(treeBranch)) {
                    WorldGen.KillTile(i, j);
                }
            }
        }
    ...
    private void GrowBigTree(int i) {
        int j = _biomeSurface.FirstOrDefault(pos => pos.X == i).Y;
        j += 1;
        if (!WorldGenHelper.GetTileSafely(i, j + 1).HasTile) {
            j = _biomeSurface.FirstOrDefault(pos => pos.X == i).Y + 1;
            j += 1;
        }
    ...
    public void BackwoodsOtherPlacements(GenerationProgress progress, GameConfiguration config) {
        ...
        foreach (Point surface in _biomeSurface) {
            for (int j = 3; j > -50; j--) {
                ...
                if (!_backwoodsPlants.Contains(tile.TileType) && !tile.ActiveTile(_elderwoodTileType) && !tile.ActiveTile(_leavesTileType)) {
                    ...
                    if (j <= 0 && tile.WallType == _dirtWallType) {
                        tile.WallType = WallID.None;
                    }
                ...
                if (tile.ActiveTile(_grassTileType) && !tile.LeftSlope && !tile.RightSlope && !tile.IsHalfBlock && !aboveTile.HasTile) {
                    ...
                    if (_random.NextChance(0.15)) {
                        ...
                        if (_random.NextBool(10)) {
                            ...
                            tile.TileFrameX = (short)(18 * (_random.Next(4) < 3 ? 2 : _random.NextBool() ? 0 : 1));
        ...
        foreach (Point surface in _biomeSurface) {
            for (int j = -_biomeHeight / 3; j < -2; j++) {
                ...
                if (SandTileTypes.Contains(tile.TileType)) {
                    WorldGenHelper.ReplaceTile(surface.X, surface.Y + j, TileID.Dirt);
                }
    ...
    private void Step1_FindPosition() {
        ...
        CenterY = BackwoodsVars.FirstTileYAtCenterBackground = WorldGenHelper.GetFirstTileY2(CenterX);
        BackwoodsVars.BackgroundOffset = 1850f + (int)(BackwoodsVars.FirstTileYAtCenter / 2f);
    ...
    private void Step3_GenerateBase() {
        ...
        cliffTileCoords.X = toLeft ? topLeftTileX - 1 : (topRightTileX + 1);
        cliffTileCoords.Y = WorldGenHelper.GetFirstTileY(cliffTileCoords.X);
        void setSurfaceY() {
        ...
        //WorldGenHelper.TileWallRunner(cliffTileCoords.X, cliffTileCoords.Y, 10, 10, TileID.IceBrick, WallID.IceBrick, true);
    ...
    private void Step4_CleanUp() {
        for (int i = Left - 35; i <= Right + 35; i++) {
            for (int j = WorldGenHelper.SafeFloatingIslandY; j < CenterY; j++) {
                ...
                if (WorldGenHelper.IsCloud(i, j) || MidInvalidTileTypesToKill.Contains(tile.TileType)) {
                    break;
                }
