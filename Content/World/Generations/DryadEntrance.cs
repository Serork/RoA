...
sealed class DryadEntrance : ModSystem {
    ...
    private static void Mountinater3(int i, int j, int denom = 4) {
    ...
    private void BuildDryadEntrance(int i, int j) {
        ...
        while (num4 > 0) {
            ...
            if (sizeValue < 4f && genRand.NextChance(value * 1.25f)) {
                sizeValue += genRand.NextFloat(0.05f, 0.1f);
            }
            num2 *= sizeValue;
            ...
            vector2D += vector2D2;
            vector2D2 = Vector2D.Lerp(vector2D2, result2, num4 / (float)size * 0.25f);
            vector2D2.X = Vector2D.Lerp(vector2D2, result2, num4 / (float)size * 0.25f).X;
        ...
        while (!flag4) {
            for (int k = killTiles.Count - 1; k > 0; k--) {
                ...
                if (Vector2.Distance(new Vector2(i, j), new Vector2((int)vector2D3.X, (int)vector2D3.Y)) > size * 1.5f) {
                    ...
                    (Main.tile[i - 1, j].TileType == tileType || Main.tile[i + 2, j].TileType == tileType)) {
                        bool flag = true;
                        ...
                        if (flag) {
                            ...
                            tile.HasTile = true;
                            tile.TileType = tileType;
                            if (WorldGenHelper.Place2x3(i, j - 1, (ushort)ModContent.TileType<TreeDryad>())) {
                                flag4 = true;
                            }
                            else {
                                tile = Main.tile[i, j];
