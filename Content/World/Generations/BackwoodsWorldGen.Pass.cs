...
sealed class BackwoodsBiomePass(string name, double loadWeight) : GenPass(name, loadWeight) {
    ...
    public static readonly ushort[] SkipBiomeInvalidWallTypeToKill = { WallID.SandstoneBrick, WallID.SmoothSandstone, WallID.HardenedSand, WallID.Sandstone, WallID.GraniteUnsafe, WallID.MarbleUnsafe, WallID.Marble, WallID.Granite };
    ...
    private void Step_AddWebs() {
        for (int num874 = 0; num874 < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 0.00003); num874++) {
                        ...
                        if (flag8 && num300 > -1 && num301 > -1 && WorldGen.countDirtTiles(num300, num301) < (flag10 ? 2000 : WorldGen.maxTileCount)) {
                            try {
                                ...
                                WorldGenHelper.CustomWall2(num300, num301, wallType, (point) => {
                                    ...
                                    WallID.Jungle1Echo, WallID.Jungle2Echo, WallID.Jungle3Echo, WallID.Jungle4Echo, WallID.Jungle);
        ...
        for (int i = origin.X - num; i <= origin.X + num; i++) {
            for (int j = origin.Y - num; j <= origin.Y + num; j++) {
                if (Main.tile[i, j].HasTile && Main.tile[i, j].TileType == TileID.LihzahrdBrick)
                    return;
                if (Main.tile[i, j].WallType == WallID.LihzahrdBrickUnsafe)
                    return;
            }
        }
        for (int i = origin.X - num; i <= origin.X + num; i++) {
    ...
    public void BackwoodsOnLast0(GenerationProgress progress, GameConfiguration config) {
        ...
        for (int num1048 = maxLeft; num1048 < maxRight; num1048++) {
            ...
            if (num1047 > 10)
                num1047 = 10;
            for (int num1052 = (int)(Main.worldSurface - 25 + num1047); num1052 < (int)(Main.worldSurface + 25 - num1047); num1052++) {
                    ...
                    if (_random.NextBool(6) && (tile.WallType == _grassWallType || tile.WallType == _flowerGrassWallType || tile.WallType == _leavesWallType)) {
                        ...
                        for (int i2 = num6; i2 <= num7; i2++) {
                            for (int j2 = num8; j2 <= num9; j2++) {
                                ...
                                if (checkTileType >= TileID.Count && TileLoader.GetTile(checkTileType) is PlantBase) {
                                    num5++;
                                }
                            }
                        }
    ...
    public void BackwoodsOtherPlacements(GenerationProgress progress, GameConfiguration config) {
        ...
        for (int i = Left - 15; i <= Right + 15; i++) {
            for (int j = WorldGenHelper.SafeFloatingIslandY; j < CenterY; j++) {
                ...
                if (Main.tile[i, j].WallType == _grassWallType && flag) {
    ...
    private void Step1_FindPosition() {
        ...
        CenterY = BackwoodsVars.BackwoodsTileForBackground = WorldGenHelper.GetFirstTileY2(CenterX, skipWalls: true);
        CenterY += _biomeHeight / 2;
