    ...
    private class NoForestPotsInBackwoods : ILoadable {
        ...
        private bool On_WorldGen_PlacePot(On_WorldGen.orig_PlacePot orig, int x, int y, ushort type, int style) {
            if (x > BackwoodsVars.BackwoodsCenterX - BackwoodsVars.BackwoodsHalfSizeX - 50 && x < BackwoodsVars.BackwoodsCenterX + BackwoodsVars.BackwoodsHalfSizeX + 50
                ...
                return WorldGen.PlacePot(x, y, (ushort)ModContent.TileType<BackwoodsPot>(), WorldGen.genRand.Next(4));
            }
            ...
            for (int j = CenterY - EdgeY; j < Bottom + EdgeY * 2; j++) {
                ...
                if ((_random.NextBool(flag3 ? 3 : 7) || (flag2 && _random.NextChance(0.2))) && WorldGen.SolidTile2(tile)) {
                    if (_random.NextBool(flag3 ? 1 : tile.TileType == _elderwoodTileType ? 2 : 4) && ((!flag3 && _random.NextChance(0.85)) || flag3) && tile.TileType != ModContent.TileType<ElderwoodDoorClosed>()) {
                        bool flag = true;
                        if (Main.tile[i, j].WallType == WallID.LihzahrdBrickUnsafe || Main.tile[i, j].WallType == WallID.LihzahrdBrick) {
                            flag = false;
                        }
                        if (flag) {
