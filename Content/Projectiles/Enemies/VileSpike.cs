	...
	public override void AI () {
		...
		if (Projectile.ai[0] % height2 == 0 && !_spawnedNext) {
			if (Main.netMode != NetmodeID.MultiplayerClient) {
				Projectile.NewProjectile(Projectile.GetSource_FromAI(), new Vector2(Projectile.Center.X, Projectile.Center.Y - 32), Vector2.Zero, ModContent.ProjectileType<VileSpike>(), Projectile.damage, Projectile.knockBack, 0, 0f, Projectile.ai[1] + 1f);
			}
			_spawnedNext = true;
		...
		if (Projectile.ai[1] >= height) {
			...
			Projectile.netUpdate = true;
			if (Main.netMode != NetmodeID.MultiplayerClient) {
				Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<VileSpikeTip>(), Projectile.damage, Projectile.knockBack);
			}
			Projectile.Kill();
		}
	}
