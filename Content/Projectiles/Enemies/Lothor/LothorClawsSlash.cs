...
sealed class LothorClawsSlash : ModProjectile {
    ...
    public override void AI() {
        ...
        if (Projectile.localAI[1] == 0f) {
            ...
            value *= 1.25f;
            Projectile.localAI[2] = Math.Max(value, 1.5f);
        }
    ...
    private void DrawItself(ref Color lightColor, float? rotation = null) {
        ...
        Rectangle r = texture.Frame(verticalFrames: 2);
        ...
        if (CanFunction()) {
            ...
            shineColor = Color.Lerp(shineColor, color2, 0f);
            ...
            spriteBatch.Draw(texture, position, new Rectangle?(texture.Frame(verticalFrames: 2, frameY: 1)), shineColor * 0.4f * num2, Projectile.rotation + (float)(Projectile.ai[2] * 0.785398185253143 * -1.0 * (1.0 - (double)num1)), origin, scale * 1.075f, effects, 0.0f);
            ...
            spriteBatch.Draw(texture, position, new Rectangle?(r), color4 * 0.15f, Projectile.rotation + Projectile.ai[2] * 0.01f, origin, scale, effects, 0.0f);
            spriteBatch.Draw(texture, position, new Rectangle?(r), color2 * num4 * num2 * 0.5f, Projectile.rotation, origin, scale * num3, effects, 0.0f);
            spriteBatch.Draw(texture, position, new Rectangle?(r), color1 * num4 * num2 * 0.625f, Projectile.rotation + (float)(Projectile.ai[2] * 0.785398185253143 * -1.0 * (1.0 - (double)num1)), origin, scale, effects, 0.0f);
            ...
            Vector2 drawpos2 = position + (Projectile.rotation + Utils.Remap(num2, 0f, 1f, 0f, (float)Math.PI / 4f) * Projectile.ai[2] - MathHelper.PiOver4 * 0.5f * Projectile.direction).ToRotationVector2() * ((float)texture.Width * 0.5915f - Utils.Remap(num1, 0f, 1f, 0f, 20f)) * scale;
