...
sealed class LothorClawsSlash : ModProjectile {
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) => target.AddBuff(BuffID.Bleeding, Main.expertMode ? 180 : 120);

    public override void OnHitPlayer(Player target, Player.HurtInfo info) => target.AddBuff(BuffID.Bleeding, Main.expertMode ? 180 : 120);
}
