...
sealed class MagicalBifrostBlock : NatureProjectile {
    ...
    public static Vector2 GetBlockPosition(Vector2 position, MagicalBifrostBlockInfo blockInfo, int offsetX = 0, int offsetY = 0, bool tiled = false) {
        Vector2 result;
        if (tiled) {
    ...
    public static MagicalBifrostBlockInfo[] GetBlockInfoForBlockType(BifrostFigureType blockType) {
        MagicalBifrostBlockInfo[] result = new MagicalBifrostBlockInfo[4];
        int currentIndexToAdd = 0;
        void addBlockInFigure(Point16 startOffset, Point16 frameCoords) {
            ref MagicalBifrostBlockInfo blockInfo = ref result[currentIndexToAdd];
            if (blockInfo.Active) {
                currentIndexToAdd++;
                return;
            }
