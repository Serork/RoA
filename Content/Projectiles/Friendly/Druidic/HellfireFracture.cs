...
sealed class HellfireFracture : NatureProjectile {
    ...
    public override void AI() {
        ...
        if (flag && proj != null) {
            ...
            if (slash != null && Projectile.ai[1] == 1f) {
                Projectile.position = proj.As<HellfireClawsSlash>().GetPos(MathHelper.PiOver4 * 0.35f);
        ...
        do {
            ...
            if (progress > 0.25f && progress < 0.9f && flag4) {
                ...
                do {
                    ...
                    if (size2 > 0f && Main.rand.NextBool(50)) {
                        Dust dust = Dust.NewDustPerfect(Vector2.Lerp(start - offset + offset2 - vector * num, start - offset + offset2 + vector * num, Main.rand.NextFloat()), 6,
                            vector.RotatedBy((float)Math.PI * 2f * Main.rand.NextFloatDirection() * 0.02f + Main.rand.NextFloatRange(MathHelper.PiOver4)) * Main.rand.NextFromList(2f, 4f) * Main.rand.NextFloat(0.35f, 0.6f) * Main.rand.NextFloat(), 0, default, (Math.Max(0.75f, size2) * 0.5f + Main.rand.NextFloatRange(0.25f)) * Main.rand.NextFloat(0.65f, 1.1f));
            ...
            vector = f.ToRotationVector2();
            if (size > minSize && Main.rand.NextBool(50)) {
                Dust dust = Dust.NewDustPerfect(Vector2.Lerp(start - offset - vector * num, start - offset + vector * num, Main.rand.NextFloat()), 6,
                    vector.RotatedBy((float)Math.PI * 2f * Main.rand.NextFloatDirection() * 0.02f + Main.rand.NextFloatRange(MathHelper.PiOver4)) * Main.rand.NextFromList(2f, 4f) * Main.rand.NextFloat(0.35f, 0.6f) * Main.rand.NextFloat(), 0, default, (Math.Max(2f, size) * 0.5f + Main.rand.NextFloatRange(0.25f)) * Main.rand.NextFloat(0.65f, 1.1f));
                dust.fadeIn = (float)(0.4 + (double)Main.rand.NextFloat() * 0.15);
                dust.noGravity = !Main.rand.NextBool(40);
            }
            start = vector2_3;
