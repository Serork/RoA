...
sealed class ShockLightning : ModProjectile {
    ...
    private readonly struct Line(Projectile projectile, Vector2 a, Vector2 b, float thickness = 1) {
        ...
        public void Draw(SpriteBatch spriteBatch, Color color = default) {
            ...
            spriteBatch.Draw(ModContent.Request<Texture2D>(ResourceManager.Textures + "Light").Value, source - Main.screenPosition, null, color.MultiplyRGB(new Color(99, 200, 204)) * 1f, rotation, middleOrigin, new Vector2(middleScale.X, middleScale.Y * 2f) * 0.02f, SpriteEffects.None, 0f);
