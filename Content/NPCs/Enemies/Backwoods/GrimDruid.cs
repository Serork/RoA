...
sealed class GrimDruid : DruidNPC {
    ...
    private void GrimDruidAttack(Vector2 position) {
        ...
        Vector2 offset = Helper.OffsetPerSolidTileSlope_Bottom(WorldGenHelper.GetTileSafely(new Point(positionX, positionY) - new Point(0, 2)));
        SoundEngine.PlaySound(SoundID.Item76, position);
        if (Main.netMode != NetmodeID.MultiplayerClient) {
            Projectile.NewProjectile(NPC.GetSource_FromAI(), new Vector2(positionX * 16f + 8 + offset.X, positionY * 16f - 30 - offset.Y), Vector2.Zero, ModContent.ProjectileType<VileSpike>(),
