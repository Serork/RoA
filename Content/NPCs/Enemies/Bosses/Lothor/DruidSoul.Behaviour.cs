...
sealed partial class DruidSoul : RoANPC {
    ...
    public override void AI() {
        ...
        if (ConsumesItself()) {
            ...
            if (player.Distance(altarPosition) > 100f) {
    ...
    private void FlyToPlayer() {
        ...
        if (Collision.CanHit(player3.Center, 0, 0, altarCoords, 0, 0) && (!Collision.CanHit(NPC.position, NPC.width, NPC.height, altarCoords - Vector2.One * 2, 4, 4) && (NPC.Center.Y > altarCoords.Y || altarStrength > 0.01f || NPC.Distance(altarCoords) > 15f))) {
    ...
    private void Movement() {
        ...
        center = NPC.Center;
        Vector2 velocity = NPC.velocity + _velocity + _velocity2 + _velocity3;
        if (Math.Abs(player3.Center.X - center.X) > 8f && Math.Abs(velocity.X) > 0.25f) {
    ...
    private void AbsorbSoulHandler() {
        ...
        _velocity2 *= 0.975f;
        //NPC.velocity *= 0.925f;
        bool flag6 = altarPosition.Y > NPC.Bottom.Y;
        if ((flag6 && !flag) || flag) {
            //_velocity.Y *= 0.95f;
            Vector2 towards2 = towards + Vector2.UnitX * 4f;
            Vector2 velocity = _velocity + _velocity2 + _velocity3 + _velocity4;
            ...
            if (!flag2) {
                ...
                Helper.InertiaMoveTowards(ref _velocity4, NPC.Center, towards);
                _velocity4 *= 0.84f;
                //_velocity3.Y *= 0.8f;
                _velocity.X *= 0.9f;
                _velocity3 *= 0.8f * (1f - altarStrength);
                _velocity3.X *= 1f - altarStrength;
                NPC.position += _velocity2 + _velocity4 * 0.3f - _velocity3;
                ...
                //    _velocity.Y *= 0.925f;
                //}
                value2 = Helper.EaseInOut4(Math.Min(_consumeValue * 3f, 1f));
                value2 = MathHelper.Clamp(value2, 0.2f, 1f);
                float value3 = (float)Math.Pow(value2, 7.0);
                float velocityY = VELOCITYY / 2f * Math.Max(value3, 0.25f + value3 * 0.5f);
                if (Math.Abs(NPC.Center.X - towards.X) < 50f) {
                    NPC.velocity.Y -= velocityY;
                }
                if (Math.Abs(NPC.Center.X - towards.X) > 15f) {
                    NPC.velocity.X += (towards - NPC.Center).SafeNormalize(Vector2.Zero).X * Math.Max(_velocity.X, 0.1f) * 0.1f; 
                }
                    ...
                    if (Main.rand.NextBool(1 + Main.rand.Next(2) + (int)((3 + count) * MathHelper.Clamp(1f - altarStrength * 3f - (1f - NPC.scale), 0f, 1f)))) {
                        ...
                        VisualEffectSystem.New<SoulPart>(VisualEffectLayer.BEHINDTILESBEHINDNPCS).
                                SetupPart(1,
                                        Vector2.Zero,
                                        position,
                                        towards + Vector2.UnitX * 4f + Main.rand.Random2(15f) + Vector2.UnitY * (10f + 20f * altarStrength),
                ...
                if (Main.rand.NextBool(3 + Main.rand.Next(2) + (int)((4 + count) * MathHelper.Clamp(1f - altarStrength - (1f - NPC.scale) - NPC.Opacity, 0.2f, 1f)))) {
                    ...
                    VisualEffectSystem.New<SoulPart>(VisualEffectLayer.BEHINDNPCS).
                            SetupPart(0,
                                    ...
                                    center - Vector2.UnitY * (flag2 ? -2f : 6f),
