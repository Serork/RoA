    ...
    private void CreateStemDusts(Vector2 currentPosition, Vector2 endPosition, float sinOffset = 0f, float extraScale = 1f) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!Main.rand.NextBool(4)) {
                Dust obj2 = Dust.NewDustPerfect(position + Main.rand.RandomPointInArea(4f), ModContent.DustType<Dusts.CarrionCane3>(), Vector2.Zero, Scale: 1.5f * Utils.Clamp(scaleProgress, 0.35f, 1f) * extraScale + 0.15f * Main.rand.NextFloat());
                obj2.noGravity = true;
                obj2.fadeIn = 0.5f;
            }
