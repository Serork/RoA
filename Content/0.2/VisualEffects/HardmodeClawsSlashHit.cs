...
sealed class HardmodeClawsSlashHit : VisualEffect<HardmodeClawsSlashHit> {
    ...
    public override void Update(ref ParticleRendererSettings settings) {
        ...
        if (Main.rand.NextChance(0.35) && TimeLeft > 15 && TimeLeft < MaxTimeLeft - 5) {
            int type = ModContent.DustType<Slash>();
            Dust dust = Dust.NewDustPerfect(Position + Main.rand.NextVector2Circular(10f, 5f), type, new Vector2?((Main.rand.NextFloat() * MathHelper.TwoPi).ToRotationVector2() * Main.rand.NextFloat(2.5f)), 0, DrawColor * 1.25f, Main.rand.NextFloat(0.75f, 0.9f) * 1.5f);
        ...
        Rotation = Utils.AngleLerp(Rotation, -0.1f + Main.rand.NextFloatRange(0.1f), 0.2f) * scale;
    ...
    public override void Draw(ref ParticleRendererSettings settings, SpriteBatch spriteBatch) {
        ...
        spriteBatch.Draw(Texture, position, null, color4, MathHelper.PiOver2 + Rotation, origin, new Vector2(Scale * scale * t3 * 0.5f, Scale), effects, 0f);
        spriteBatch.Draw(Texture, position, null, color3, Rotation, origin, new Vector2(Scale, Scale * scale), effects, 0f);
        spriteBatch.Draw(Texture, position, null, color3, MathHelper.PiOver2 + Rotation, origin, new Vector2(Scale * scale * t3 * 0.5f, Scale) * 0.6f, effects, 0f);
        spriteBatch.Draw(Texture, position, null, color4, Rotation, origin, new Vector2(Scale, Scale * scale) * 0.6f, effects, 0f);

        spriteBatch.Draw(Texture, position, null, color * 0.5f, MathHelper.PiOver2 + Rotation, origin, new Vector2(Scale * scale * t3, Scale), effects, 0f);
        spriteBatch.Draw(Texture, position, null, color * 0.5f, Rotation, origin, new Vector2(Scale, Scale * scale), effects, 0f);
        spriteBatch.Draw(Texture, position, null, color * 0.5f, MathHelper.PiOver2 + Rotation, origin, new Vector2(Scale * scale * t3, Scale) * 0.6f, effects, 0f);
        spriteBatch.Draw(Texture, position, null, color * 0.5f, Rotation, origin, new Vector2(Scale, Scale * scale) * 0.6f, effects, 0f);
