...
sealed class CaptureBiomeSettings : ModSystem {
    ...
    internal static int biomeCaptureCount = 1;
    ...
    internal void CaptureIconSetInfo(int captureIconID, Rectangle iconHitbox, Point mouse, ref Texture2D iconTexture, ref int textureXposition) {
        if (captureIconID == biomeCapturesIndexs[0]) {
            ...
            if (iconHitbox.Contains(mouse)) {
                if (captureIconID == biomeCapturesIndexs[0]) {
                    Main.instance.MouseText(Language.GetTextValue("Mods.RoA.World.BackwoodsBGIcon"), 0, 0); //text for our icon
                }
            }
        }
    }
...
{
            return new CaptureBiome(ModContent.GetInstance<BackwoodsBackgroundSurface>().Slot, ModContent.GetInstance<DruidBiomeWaterStyle>().Slot); //create a new capture biome
        }
        return null;
    }
    ...
    private void moveCaptureDefaultsHitbox(ILContext il) {
        ...
        c.EmitDelegate((int i, ref Rectangle rectangle, int y) => {
            int y2 = y + i * 20;
            if (i == 6) {
                rectangle.Y = y2 + 24 * ((int)Math.Ceiling((maxModdedCaptureCounts + 13.00) / 7.00) - 2);
            }
        });
    }
    ...
    private void moveCaptureDefaultsText(ILContext il) {
        ...
        c.EmitDelegate((int i, ref Rectangle rect, object self) => {
            ...
            if (i == 6) {
                rect.Y = rect2.Y + 24 * ((int)Math.Ceiling((maxModdedCaptureCounts + 13.00) / 7.00) - 2);
            }
        });
    ...
    private Rectangle GetRect() {
        ...
        if (maxModdedCaptureCounts > 0) {
            result.Height = 170 + 24 * ((int)Math.Ceiling((maxModdedCaptureCounts + 13.00) / 7.00) - 2); //offset the back panel
        }
        return result;
    }
    ...
    private Rectangle increaseCaptureSettingsHeight(On_CaptureInterface.ModeChangeSettings.orig_GetRect orig, object self) {
        ...
        if (maxModdedCaptureCounts > 0) {
            rect.Height = 170 + 24 * ((int)Math.Ceiling((maxModdedCaptureCounts + 13.00) / 7.00) - 2); //offset the back panel
        }
        return rect;
    }
    ...
    private void DrawModdedCaptureIcons(ILContext il) {
        ...
        c.EmitDelegate((int i, Rectangle r, int j) => {
            return i >= (int)Math.Ceiling((r.X + 13.00) / 7.00) - 1 && j >= r.X + 13 - i * 7; //the if statement for the continue statement, this makes all other icons outside of the bounds of whats added to be not drawn
        });
        ...
        {
            if (i >= (int)Math.Ceiling((maxModdedCaptureCounts + 13.00) / 7.00) - 1) //if at the last row
            {
                return (int)(24 * ((7 - (maxModdedCaptureCounts + 13 - i * 7)) / 2.00)); //return offset based on the final row count
            }
        ...
        {
            return (int)Math.Ceiling((r.X + 13.00) / 7.00); //calculate the number of rows there are depending on the count of icons
        });
    }
}
