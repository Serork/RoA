...
sealed partial class PlayerCommon : ModPlayer {
    ...
    public void CollideWithStarwayWormhole(StarwayWormhole starwayWormhole, bool reversed = false) {
        ...
        for (int i = 0; i < 20; i++) {
            int size = 15;
            int num492 = Dust.NewDust(Player.position - Vector2.One * size / 2f, Player.width + size, Player.height + size, Main.rand.NextBool() ? ModContent.DustType<FilamentDust>() : ModContent.DustType<StarwayDust>(), 0f, 0f, 0, default(Color), 2.7f * 1f);
            Main.dust[num492].noGravity = true;
            Main.dust[num492].velocity += Player.velocity * 0.25f * Main.rand.NextFloat(1f, 2f);
        }
    }
