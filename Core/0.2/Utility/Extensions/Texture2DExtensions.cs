...
static class Texture2DExtensions {
    public static List<Vector2> GetColorMap(this Texture2D texture, Vector2 texturePosition, float textureRotation, float textureScale, float distanceToPreviousToBeAdded = 0f, float chance = 1f, bool addOffset = true) {
        ...
        for (int h = 0; h < textureHeight; h++) {
            for (int w = 0; w < textureWidth; w++) {
                ...
                if (color.A > 0 && Main.rand.NextChance(chance)) {
                    ...
                    if (previousPosition.Distance(position) < distanceToPreviousToBeAdded) {
                        continue;
                    }
                    result.Add(position);
