...
static class ProjectileExtensions {
    ...
    public static void EmitEnchantmentVisualsAtForNonMelee(this Projectile projectile, Vector2 boxPosition, int boxWidth, int boxHeight) {
        ...
        if (player.frostBurn && projectile.friendly && !projectile.hostile && !projectile.noEnchantments && Main.rand.Next(2 * (1 + projectile.extraUpdates)) == 0) {
        ...
        if (player.magmaStone && !projectile.noEnchantments && Main.rand.Next(3) != 0) {
